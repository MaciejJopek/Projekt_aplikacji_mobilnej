...
public class hantlewykres extends AppCompatActivity {
    ...
    public void SaveToDatabase(){
        ...
        db.saveDate(xvalue,yvalue, "Hantle");
        addDataToGraph();
        barChart.invalidate();
    ...
    public void addDataToGraph(){
        ...
        final ArrayList<String> ydata = db.queryYData("'Hantle'");

        for (int i = 0;i<db.queryYData("'Hantle'").size();i++){
            BarEntry newBarEntry = new BarEntry(i, Float.parseFloat(db.queryYData("'Hantle'").get(i)));
            yVals.add(newBarEntry);
        }
        final ArrayList<String> xVals = new ArrayList<String>();
        final ArrayList<String> xdata = db.queryXData("'Hantle'");

        for (int i = 0; i < db.queryXData("'Hantle'").size();i++){
